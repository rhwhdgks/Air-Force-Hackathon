...
ALL   = pd.read_json(os.path.join(MAPDIR, 'pred_50m_50cap.json'))
...
TR0 = ALL.trashcan.values.astype(np.float32)
TR  = np.minimum(TR0, W)

N_PT = len(ALL)
TOTAL_WASTE = int(np.sum(W))

cand_idx  = np.arange(N_PT, dtype=np.int32)      # 0 … N_PT-1
cand_mask = np.ones(N_PT,  dtype=bool)           # 전부 True
...
# 정적 라우트
@app.route('/')
def index(): return send_from_directory(STATIC, '7.2완성일까.html')
...
def run_greedy_stream():
    ...
    def total_reduction():                 
        before_cap = np.minimum(TR * (1.0 + np.log1p(assigned)), W)
        return float(np.sum(before_cap))    
    def add(i):
    ...
    def greedy():
        for i in range(P):
            ...
            factor = 1.0 - 1.0 / (1.0 + np.log1p(assigned))
            a = assigned
            after  = np.minimum(TR * (1.0 + np.log1p(a + 1)), W)
            before = np.minimum(TR * (1.0 + np.log1p(a    )), W)
            gains  = np.where(valid, after - before, -1)
            j=int(gains.argmax())
            if gains[j]<=0: break
            add(j)
    ...
    def gain(i): 
        nb = neighbours(i, R_eff_m, Reff_lat)
        a  = assigned[nb]
        after  = np.minimum(TR[nb] * (1.0 + np.log1p(a + 1)), W[nb])
        before = np.minimum(TR[nb] * (1.0 + np.log1p(a    )), W[nb])
